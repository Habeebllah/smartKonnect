...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomUser
        fields = ('id','email', 'username','FullName','pin','img',"Address",'Phone','user_type',"email_verify","password",'Account_Balance','wallet_balance','bonus_balance','referer_username','bank_accounts',"reservedaccountNumber","reservedbankName")
...
class bonus_transferSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if float(amount) > user.Referer_Bonus:
            ...
            errors['error'] = u'BELOW MINIMUM AMOUNT ALLOWED (N200)'
            raise serializers.ValidationError(errors)
...
class WithdrawSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if float(amount) >user.Account_Balance:
            errors['error'] = u' insufficient balance  N{} '.format(user.Account_Balance)
            raise serializers.ValidationError(errors)

        elif float(amount) < 1000:
            errors['error'] = u' Minimun withdraw is #1000 per transaction'
            raise serializers.ValidationError(errors)

        elif float(amount) > 5000:
            errors['error'] = u' Maximum withdraw is #5000 per transaction'
            raise serializers.ValidationError(errors)
        ...
        elif config.disable_Transaction_limit == False and user.is_superuser==False and user.verify == False and config.disable_Transaction_limit == False  and  float(amount) > config.unverified_users_daily_withdraws_limit:
            errors['error'] = u'Unverified User are limited to {0} naira withdraw  per day'.format(config.unverified_users_daily_withdraws_limit)
            raise serializers.ValidationError(errors)


        # elif Transactions.objects.filter(user=user,transaction_type="DEBIT",create_date__date=datetimex.today()).exists():
        #     if user.verify == False and config.disable_Transaction_limit == False  and Transactions.objects.filter(user=user,transaction_type="DEBIT",create_date__date=datetimex.today()).aggregate(Sum('amount'))['amount__sum'] > 10000:
        #         errors['error'] = u'You have exceed  daily transactions limit for unverify account (N10000 ), pls verify your account to continue your daily transaction'
        #         raise serializers.ValidationError(errors)
...
class Recharge_pin_orderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('user','network','network_amount','name_on_card','quantity','data_pin','id','Status',"previous_balance","after_balance","amount",'create_date' )
    ...
    def validate(self, data):
        ...
        elif config.disable_Transaction_limit == False and user.verify == False and config.disable_Transaction_limit == False  and  float(amt) > config.unverified_users_daily_transation_limit:
            errors['error'] = u'Unverified User are limited to N{} naira airtime topup  per day'.format(config.unverified_users_daily_transation_limit)
            raise serializers.ValidationError(errors)
...
class Result_Checker_Pin_orderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('user','exam_name','pins','quantity','id','Status',"previous_balance",'data',"after_balance","amount",'create_date' )
...
class TransferSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if float(amount) >user.Account_Balance:
            ...
            errors['error'] = u'BELOW MINIMUM AMOUNT ALLOWED (N100)'
            raise serializers.ValidationError(errors)


        elif config.disable_Transaction_limit == False and user.is_superuser==False and user.verify == False and config.disable_Transaction_limit == False  and  float(amount) > config.unverified_users_transfer_limit:
            errors['error'] = u'Unverified User are limited to {0} naira transfer  per day'.format(config.unverified_users_transfer_limit)
            raise serializers.ValidationError(errors)
...
class BillpaymentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if paid_amount > user.Account_Balance:
            errors['error'] = u'insufficient balance,  your current balance N{}  '.format(user.Account_Balance)
            raise serializers.ValidationError(errors)
            ...
            errors['error'] = u'BELOW MINIMUM AMOUNT ALLOWED (N500)'
            raise serializers.ValidationError(errors)

        elif config.disable_Transaction_limit == False and user.verify == False and config.disable_Transaction_limit == False  and  float(data.get('amount')) > config.unverified_users_daily_transation_limit:
            errors['error'] = u'Unverified User are limited to N{} naira airtime topup  per day'.format(config.unverified_users_daily_transation_limit)
            raise serializers.ValidationError(errors)

        elif Billpayment.objects.filter(user=user,create_date__date=datetimex.today()).exists():
            if user.verify == False and config.disable_Transaction_limit == False  and Billpayment.objects.filter(user=user,create_date__date=datetimex.today()).aggregate(Sum('amount'))['amount__sum'] > 10000:
                errors['error'] = u'You have exceed  daily transactions limit for unverify account (N10000 ), pls verify your account to continue your daily transaction'
                raise serializers.ValidationError(errors)
...
class DataSerializer(ModelSerializer):
    ...
    class Meta:
        model = Data
        fields =  ('user','id','ident', 'customer_ref','network','balance_before','balance_after','mobile_number','plan','Status','plan_network','plan_name','plan_amount','create_date',"Ported_number")
    ...
    def validate(self, data):
        ...
        #     raise serializers.ValidationError(errors)
            ...
            errors['error'] = u'You can\'t purchase this plan due to insufficient balance  N{} Kindly Fund your Wallet'.format(user.Account_Balance)
            raise serializers.ValidationError(errors)

        elif config.disable_Transaction_limit == False and user.verify == False and config.disable_Transaction_limit == False  and  float(amount) > config.unverified_users_daily_transation_limit:
            errors['error'] = u'Unverified User are limited to {0} naira datatopup topup  per day'.format(config.unverified_users_daily_transation_limit)
            raise serializers.ValidationError(errors)
...
class AirtimeTopupSerializer(ModelSerializer):
    ...
    class Meta:
        model = AirtimeTopup
        fields =  ('user','id','airtime_type','ident','network','airtime_type','paid_amount','mobile_number','amount','plan_amount','plan_network','balance_before','balance_after','Status','create_date',"Ported_number")
    ...
    def validate(self, data):
        ...
        if float(amount) >user.Account_Balance:
            errors['error'] = u'You can\'t topup due to insufficient balance  N{} '.format(user.Account_Balance)
            raise serializers.ValidationError(errors)

        elif float(data.get('amount')) < 50:
            errors['error'] = u'minimum airtime topup is N50'
            raise serializers.ValidationError(errors)

        elif config.disable_Transaction_limit == False and user.verify == False and config.disable_Transaction_limit == False  and  float(amount) > config.unverified_users_daily_transation_limit:
            errors['error'] = u'Unverified User are limited to N{0}  airtime topup  per day'.format(config.unverified_users_daily_transation_limit)
            raise serializers.ValidationError(errors)

        elif airtime_type == "Share and Sell" and float(data.get('amount')) < 100:
            errors['error'] = u'minimum airtime share and sell topup is N100'
            raise serializers.ValidationError(errors)
        ...
        elif airtime_type == "awuf4U" and Network.objects.get(name=net.name).awuf4u_disable == True:
                      errors['error'] = u'Awuf4u airtime is not available on this network currently'
                      raise serializers.ValidationError(errors)
            ...
            if user.verify == False and config.disable_Transaction_limit == False  and AirtimeTopup.objects.filter(user=user,create_date__date=datetimex.today()).aggregate(Sum('amount'))['amount__sum'] > 10000:
                errors['error'] = u'You have exceed  daily transactions limit for unverify account (N10000 ), pls verify your account to continue your daily transaction'
                raise serializers.ValidationError(errors)
