...
class CustomUser(AbstractUser):
    ...
    def h_account(self):
        try:
            return json.loads(self.accounts2)
        except:
            pass
    ...
    def passport(self):
        ...
        if self.passport_photogragh:
            return "https://www.yuniqtelecoms.com/media/%s" % (self.passport_photogragh)
        else:
    ...
    def image_tag(self):
        ...
        from django.utils.html import mark_safe
        return mark_safe('<img src="https://www.yuniqtelecoms.com/media/%s" width="150" height="150" />' % (self.passport_photogragh))
...
class WebsiteConfiguration(models.Model):
    ...
    intro_message = models.TextField(default="Refer people to yuniqtelecoms and earn N500 immediately the person upgrade his/her account to affiliate or topuser",null=True, blank=True)
    ...
    easyaccess_api_key = models.CharField(max_length=500, null=True, blank=True, help_text="Enter your APIKEY here if you are using easyaccess.com.ng")
    bulksms9ja_api_token = models.CharField(max_length=500, null=True, blank=True, help_text="Enter your API Token here if you are using bulksmsnigeria.com")
    smartsms_api_token = models.CharField(max_length=500, null=True, blank=True, help_text="Enter your API Token here if you are using smartsmssolutions.com")
    ...
    Cable_provider = models.CharField(max_length=50, blank=True, null=True, default="VTPASS", choices=( ('VTPASS','VTPASS'),('RINGO','RINGO'), ('MSORG WEB','MSORG WEB'), ('SMS','SMS')))
    Bill_provider = models.CharField(max_length=50, blank=True, null=True, default="VTPASS", choices=( ('VTPASS','VTPASS'),('RINGO','RINGO'), ('MSORG WEB','MSORG WEB'), ))
    BulkSMS_provider = models.CharField(max_length=50, blank=True, null=True, default="HOLLATAGS", choices=( ('VTPass','VTPass'),('HOLLATAGS','HOLLATAGS'),('BulkSMS9ja','BulkSMS9ja'), ('SMART_SMS_SOLUTION','SMART_SMS_SOLUTION'),))
    ...
    email_verification = models.BooleanField(max_length=50, default=False, choices=( (True,'ENABLE'),(False,'DISABLE'), ), verbose_name="Disable Transaction limit for unverified user")
...
class Network(models.Model):
    ...
    gifting_vending_medium = models.CharField(max_length=30, choices=api_medium,blank=True, null=True)
    ...
    msorg_web2_net_id = models.CharField(max_length=5, blank=True, null=True)
    msorg_web3_net_id = models.CharField(max_length=5, blank=True, null=True)
    ...
    network_logo = models.ImageField(upload_to = "network_logo",null=True,blank=True)
    ...
    def image_tag(self):
        if self.network_logo:
            return mark_safe('<img src="%s" style="width: 45px; height:45px;" />' % self.network_logo.url)
        else:
            return 'No Image Found'
    image_tag.short_description = 'Network Logo'
...
class Plan(models.Model):
    ...
    ogdams_plan_id = models.CharField(max_length = 30,blank=True,null=True)
...
class Result_Checker_Pin(models.Model):
    ...
    provider_api = models.CharField(max_length=30, default="MOBILENIG",choices=(("MSORG WEB","MSORG WEB"),("MOBILENIG","MOBILENIG"),("VTPASS","VTPASS")))
    # provider_api = models.CharField(max_length=30, default="MOBILENIG",choices=(("MSORG WEB","MSORG WEB"),("MOBILENIG","MOBILENIG"),("VTPASS","VTPASS"),("EASYACCESS","EASYACCESS.COM.NG")))
...
class Result_Checker_Pin_order(models.Model):
    ...
    def purchase_pin(self):
        if self.data:
            data =  json.loads(self.data)
            if "details" in data :
                if self.exam_name == "NECO":
                    if "tokens"  in data['details']:
                        tokens = ""
                        for x in data['details']["tokens"]:
                            print(x['token'] )
                            #str(data['details']["tokens"])
                            tokens = tokens  + "token -> " + str(x['token']) + ","
                        return tokens
                    else:
                           return " "


                elif self.exam_name == "WAEC":
                    if "pins"  in data['details']:
                        # return str(data['details']["pins"])

                        tokens = ""
                        for x in data['details']["pins"]:


                            tokens = tokens  + 'serial_number -> ' + str(x['serial_number']) + ':  pin -> ' + str(x['pin']) + ","
                        return tokens
...
class Data(models.Model):
    ...
    customer_ref = models.CharField(max_length=100, blank=True, null=True, verbose_name="Customer Ref")
    ...
    def requery(self):
        ...
        try:
            if self.provider_source == "VTPASS":
                result = 'unable to fetch response from provider'
                
            elif self.provider_source == "SIMHOST":
                """
                Code  Meaning
                0	Pending
                ...
                logging.info('')
                logging.info('')
                logging.info('---------------------------------------------- QuerySimhosting --------------------------')
                logging.info('')
                logging.info('')
                logging.info('')
                url = "https://ussd.simhosting.ng/api/status/"
                payload={
                ...
                logging.info(f'result = {raw_data}')
                
                if 'data' in raw_data:
                    resp = raw_data["data"]["response"]
                    logging.info(f'resp = {resp}')
                    
                    if int(raw_data['data']['code']) == 3:
                        result = f"TRANSACTION FAILED:  {raw_data['data']['response']}" +  f"\n \n status code: {raw_data['data']['code']}"
                    else:
                        result = raw_data['data']['response'] + f"\n \n status code: {raw_data['data']['code']}"
                        
                else:
                    result = "Invalid log_id supplied"
                validity = True
        ...
        return {'msg': str(result), 'valid':validity}
    ...
    def save(self, *args, **kwargs):
        ...
        if self.Status == 'failed' and self.refund == False:
            ...
            after_bal = self.user.Account_Balance + float(self.plan_amount)
                ...
                if self.user.referer_username:
                    if CustomUser.objects.filter(username__iexact = self.user.referer_username).exists():
                        ...
                        if com > 0:
                            ...
                            Wallet_summary.objects.create(user= referer, product="[Referal bonus] refund  N{} commission for your referal failed {} DATA order".format(self.plan.commission,self.user.username), amount = self.plan.commission, previous_balance = com, after_balance= (com - self.plan.commission))
                            notify.send(referer, recipient=referer, verb="[Referal bonus] refund N{}commission  for your referal failed {} DATA order ".format(self.plan.commission,self.user.username))
            except:
                pass
        ...
        try:
            if self.user.webhook  and self.id:
                # jsondata = seria2.serialize('json', [Data.objects.get(id=self.id)])
                # data = json.loads(jsondata)[0]["fields"]

                data = {
                    "network":self.network.name, 
                    "data_type":self.plan.plan_type, 
                    "purchased_plan":str(self.plan.plan_size) +  self.plan.plan_Volume, 
                    ...
                    "transaction_date":self.create_date.strftime('%d, %b %Y')
                }                   
                
                headers = { 'Content-Type': 'application/json'}
                response = requests.post(self.user.webhook,data=json.dumps(data),headers=headers)
                print(response.text)
        except:
            pass
        ##############send webhook section end###############
    ...
    def requery(self):
        try:
            if self.provider_source == "VTPASS":
                ...
                result = a["response_description"]
                validity = True
                
            elif self.provider_source == "SIMHOST":
                print('---------------------------------------------- QuerySimhosting --------------------------')
                url = "https://ussd.simhosting.ng/api/status/"
                payload={
                    'token': f'{config.simhost_API_key}',
                    'log_id': self.ident
...
class KYC(models.Model):
    ...
    def __str__(self):
        return '<img src="https://www.yuniqtelecoms.com/media/{}" height="150"/>'.format(self.passport_photogragh.url)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.Status == 'failed' and self.refund == False:
            ...
            Wallet_summary.objects.create(user=self.user, product="{} {}Bill Payment Refund for {} ".format(
                ...
                sendmessage('BILLTOKEN', "From yuniqtelecoms your Bill Token is {0} ".format(
